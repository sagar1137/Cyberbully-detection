...
def handle_client(c, addr):
    ...
    def send():
        ...
        if varContent and (not varContent.isspace()):
            result=detect(varContent)
            length=len(varContent.split())
            if result == "POSITIVE" or length<2:
                ...
                messagebox.showwarning("Warning","Please be more polite")
    ...
    def FileTransfer():
        ...
        if File_path:
            ...
            if(maping[new_ans] == "Porn" or maping[new_ans] == "Sexy"):
                messagebox.showwarning("Warning","Inappropriate image Detected. Please Send Different Image")
            else:
